...
// on message receive
  ws.on('message', function (message) {
    ...
    //alicenode
    if (message.includes('alicenodefileRequest')) {
      fileName = message.replace('alicenodefileRequest', '')
      // send the content of the file to the client editor
      ws.send('currentVersion?' + fs.readFileSync("../alicenode/" + fileName, 'utf8'))

      // get all of the commits which contain the file
      exec('git log --all --source --abbrev-commit --pretty="%h | %cr | %cn | %B" -- ' + fileName, {cwd: serverPath}, (stdout, stderr, err) => {
        // console.log(JSON.stringify(stderr))
        let commitList = stderr.split('refs/heads').join('')
        ws.send('branchCommits?' + commitList)
        console.log(commitList)
    ...
    // /alicenode
    if (message.includes('getCurrentBranch')) {
      exec('git rev-parse --abbrev-ref HEAD', { cwd: serverPath }, (stdout, stderr, err) => {
        // console.log("this it sshshs kjdlfj;ldkslfj" + stderr.replace(" string", ""));
        ws.send('branchname?' + stderr.replace('\n', ''))
        // console.log("branchname?" + stderr.replace("\n", ""))
      })
    ...
    if (message.includes('git show')) {
      ...
      if (fileName) {
        // path.join("..", "alicenode_inhabitat/project.cpp")
        exec(message + ':' + fileName, { cwd: serverPath }, (stdout, stderr, err) => {
          console.log("stdout" + stdout)
          console.log("stderr" + stderr)
          console.log("err" + err)
          ws.send('show?' + stderr)
      ...
      // this is run when a client connects. its a bit of a lgeacy feature left over from the earliest version of the client-server.
        ...
        // path.join("..", "alicenode_inhabitat/project.cpp")
        exec(gitCommand, { cwd: serverPath }, (stdout) => {
          ws.send('show?' + stdout)
    ...
    if (q > 0) {
      ...
      switch (cmd) {
        ...
        case 'currentBranch':
          ...
          break
          //alicenode
          case 'alicenodecurrentBranch':
          // console.log("\n\n\n\n\n" + userWorktree)
          exec('git checkout ' + arg, {cwd: serverPath}, (stdout, stderr, err) => {
            console.log(err)

            // once the userWorktree is pointed at a branch, retrieve all files on the branch and send to client
            fileList = fs.readdirSync(serverPath).filter(function (file) {
              if (file.charAt(0) === '+');
              else if (file.charAt(0) === '.');
              else if (file.includes('userlist.json'));
              else if (file.includes('userWorktree'));
        ...
        case 'edit':
          ...
          fs.writeFileSync(serverPath + '/' + fileName, newCode, 'utf8')
          // git add and commit the new changes, including commitMsg
          execSync('git add .', {cwd: serverPath }, () => { console.log('git added') })
          execSync('git commit --author=\"' + thisAuthor + '\" -m \"' + commitMsg + '\"', {cwd: serverPath }, () => { console.log('git committed') })
          execSync('git status', {cwd: serverPath }, (stdout) => { console.log('\ngit status: \n' + stdout) })
